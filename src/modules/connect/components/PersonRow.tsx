...
export function PersonRow({ person, onPreview, action, note, variant = 'directory' }: { person: Person; onPreview: (person: Person) => void; action: ReactNode; note?: string; variant?: 'directory'|'mentor'|'compact' }) {
  return <li className={'person-row person-row--'+variant} data-person-id={person.id}>
    <button className="person-avatar-button" type="button" onClick={()=>onPreview(person)} aria-label={'Preview '+person.name}><Avatar name={person.name} personId={['aarav','ishita','rohan','kavya','meera','arjun','nikhil','sara'].includes(person.id)?person.id:undefined}/></button>
    ...
    <div className="person-action">{action}</div>{variant==='directory'&&<Dropdown label={'More about '+person.name} trigger={<Icon name="more"/>} items={[{label:'View profile',action:()=>onPreview(person)}]}/>}
  </li>;
}
